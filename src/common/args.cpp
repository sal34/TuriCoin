...
const char * const BITCOIN_CONF_FILENAME = "turicoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\TuriCoin
    //   new: C:\Users\Username\AppData\Local\TuriCoin
    // macOS: ~/Library/Application Support/TuriCoin
    // Unix-like: ~/.turicoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "TuriCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "TuriCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/TuriCoin";
#else
    // Unix-like
    return pathRet / ".turicoin";
#endif
#endif
}
